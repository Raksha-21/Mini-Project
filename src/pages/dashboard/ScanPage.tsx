...
export default function ScanPage() {
  ...
  setStream(null);
  setMode("upload"); // optional: switch UI after capture
};
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files?.[0]) {
      ...
      stopCamera(); // ensure camera stops if switching
    }
  };
  ...
  const handleDetect = async () => {
    ...
    await fetch("/api/detect-disease", { method: "POST", body: formData });
  };
  ...
  return (
    <DashboardLayout>
      <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center bg-gradient-to-br from-green-100 via-emerald-100 to-green-50">
        <div className="rounded-3xl shadow-2xl p-8 sm:p-10 w-[90%] max-w-2xl bg-white border border-green-300 flex flex-col items-center space-y-6">
          <h2 className="text-2xl sm:text-3xl font-extrabold text-center text-green-800">
            🌿 Scan & Detect Crop Disease
          </h2>
